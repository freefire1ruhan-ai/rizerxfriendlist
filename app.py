...
# Endpoints:
#   GET /<JWT>                                         -> friend list using JWT
#   GET /guest?uid=<guest_uid>&password=<guest_password> -> friend list from guest login
#   GET /access_token?access_token=<access_token>     -> friend list from access token
#   GET /                                              -> usage info
...
# ================================
# 1. Friend list part (multi‑server, multi‑threaded)
# ================================
...
def get_friend_list_from_jwt(jwt):
    """Multi-threaded friend list fetcher. Returns (friends_list, my_info) or (None, error)."""
...
# ================================
# 2. Guest login (uid + password → JWT → friend list)
# ================================
...
# Embedded MajorLoginReq protobuf (full definition)
...
_login_res_b64 = "ChNNYWpvckxvZ2luUmVzLnByb3RvInwKDU1ham9yTG9naW5SZXMSEwoLYWNjb3VudF91aWQYASABKAQSDgoGcmVnaW9uGAIgASgJEg0KBXRva2VuGAggASgJEgsKA3VybBgKIAEoCRIRCgl0aW1lc3RhbXAYFSABKAMSCwoDa2V5GBYgASgMEgoKAml2GBcgASgMYgZwcm90bzM="
...
import base64
_sym_db = _symbol_database.Default()
DESCRIPTOR = _descriptor_pool.Default().AddSerializedFile(base64.b64decode("{_login_req_b64}"))
_globals = globals()
...
import base64
_sym_db = _symbol_database.Default()
DESCRIPTOR = _descriptor_pool.Default().AddSerializedFile(base64.b64decode("{_login_res_b64}"))
_globals = globals()
_builder.BuildMessageAndEnumDescriptors(DESCRIPTOR, _globals)
    ...
    try:
        ...
        if len(data) % 16 == 0:
            ...
            if dec:
                ...
                if res.token:
                    return True, {'account_uid': res.account_uid, 'region': res.region, 'token': res.token, 'url': res.url, 'timestamp': res.timestamp, 'key': res.key.hex() if res.key else None, 'iv': res.iv.hex() if res.iv else None}
        res = MajorLoginRes()
...
# ================================
# 3. Access token → JWT → friend list (new combined endpoint)
# ================================
...
def major_login_with_platform(open_id, access_token, platform_type):
    """Same as major_login but sets origin_platform_type and primary_platform_type to platform_type"""
    # Build message with platform override
    msg = MajorLoginReq()
    msg.event_time = str(datetime.now())[:-7]
    msg.game_name = "free fire"
    msg.platform_id = 1
...
def home():
    return jsonify({
        ...
        "examples": {
            "guest": "/guest?uid=1234567890&password=mypass",
            "access_token": "/access_token?access_token=abc123..."
        }
    })
